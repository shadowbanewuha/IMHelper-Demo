...
bool CMenuUI::SelectRange(int iIndex, bool bTakeFocus)
{
	return true;
}
...
CMenuWnd::CMenuWnd(CControlUI *pRootControl):
m_pRootControl(pRootControl),
m_hParent(pRootControl->GetManager()->GetPaintWindow()),
m_pOwner(NULL),
m_pLayout(),
m_xml(_T(""))
{}
...
void CMenuWnd::Notify(TNotifyUI& msg) {
	if (msg.sType == _T("itemclick")) {
		m_pRootControl->GetManager()->SendNotify(msg.pSender, _T("menu_item_click"), msg.wParam, msg.lParam, true);
	}
}
...
SIZE CMenuElementUI::EstimateSize(SIZE szAvailable)
{
	...
	if(cXY.cy == 0) {
		...
		RECT rcText = { 0, 0, std::fmax(szAvailable.cx, m_cxyFixed.cx), 9999 };
		rcText.left += pInfo->rcTextPadding.left;
		rcText.right -= pInfo->rcTextPadding.right;
		if( pInfo->bShowHtml ) {   
			int nLinks = 0;
...
void CMenuElementUI::CreateMenuWnd()
{
	...
	m_pWindow = new CMenuWnd(m_pManager->GetRoot());
	ASSERT(m_pWindow);
